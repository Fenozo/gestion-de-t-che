var affectationController = function($scope,api,root){
    ...
    $scope.afficher_affectation = true;
...
$scope.openWindow =  function (){
    ...
    $scope.valider_affectation = function($object){
        if($object){
            
             if($object.id_affectation != -1){
                 console.log($object)
                $object.id_affectation = -1;
                api.update('equipement',$object).then(function(r){
                console.log(r);
            });
            }
        }
        angular.forEach($scope.objectAffecter,function(current,key){
            // si le id_affectation != -1 alors id_affectattion = -1
